...
## 📊 Bemærkning om data
Det bagvedliggende datasæt (`spx_total_return.csv`), som anvendes i projektet, **ikke** inkluderet i dette offentlige repository (styret via `.gitignore`). For at køre koden lokalt, kræves en tilsvarende CSV-fil placeret i projektets rodmappe.
...
* `1_dags VaR.py`: Beregner de estimerede VaR- og ES-værdier for både de statiske modeller og GARCH-modellerne. Scriptet plotter desuden Q-Q plots for de standardiserede residualer og udfører en vægtet Ljung-Box test på de standardiserede residualer.
