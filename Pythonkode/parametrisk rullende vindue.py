...
# 1. Find stien til Pythonkode-mappen (der hvor scriptet kører fra)
script_mappe = os.path.dirname(os.path.abspath(__file__))
# 2. Gå ét niveau OP (fra 'Pythonkode' til 'Bachelor'-mappen)
bachelor_mappe = os.path.dirname(script_mappe)
# 3. Peg på 'Grafer'-mappen, som ligger i Bachelor-mappen
grafer_mappe = os.path.join(bachelor_mappe, "Grafer")

# Tjek at stien virker
if not os.path.exists(grafer_mappe):
    os.makedirs(grafer_mappe)

# ---------------------------------------------------------
# 1. Hent og klargør data
# ---------------------------------------------------------
print("Henter data...")
...
def backtest_tests(hits, p, model_navn):
    ...
    # --- JORION NOTATION ---
    T = len(hits)          # Total antal observationer
...
# ---------------------------------------------------------
# 3. Træn den RULLENDE Parametriske Model (Normal + t-fordeling)
# ---------------------------------------------------------
...
# B) T-FORDELING VaR
# Estimer frihedsgrader (df) på træningsdata
...
# ---------------------------------------------------------
# 4. Klargør Testdata (De sidste 510 dage)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 5. Visualisering til Bacheloropgaven
# ---------------------------------------------------------
...
# Fordi disse er mindre (s=25), vil de dage, hvor BEGGE er brudt, ligne en orange prik med en tyk blå kant!
plt.scatter(breaches_t.index, breaches_t, color='purple', s=25, zorder=6, 
            label=f"Overskridelser: Statisk-t ({antal_breaches_t} stk)")

plt.title("Backtest af Rullende Statisk VaR (Normal vs. Student-t)")
plt.xlabel("Dato")
plt.ylabel("Dagligt Log-afkast")

# --- NYT: Skab plads i bunden til infoboksen ---
# Vi finder det laveste afkast og skubber y-aksen 0.025 længere ned
plt.ylim(test_returns.min() - 1, test_returns.max() + 1)

# framealpha=0.95 gør boksen hvid og solid, så streger ikke går igennem teksten
...
# Udregn 'hits' (overskridelser) for begge modeller
